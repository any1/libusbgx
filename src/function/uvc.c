...
static int uvc_set_frame(char *format_path, const char *format, const struct usbg_f_uvc_frame_attrs *attrs)
{
	...
	ret = usbg_write_dec64(frame_path, frame_name, "dwMaxVideoFrameBufferSize", attrs->dwMaxVideoFrameBufferSize);
	if (ret != USBG_SUCCESS)
		return ret;

	ret = usbg_write_dec64(frame_path, frame_name, "dwMinBitRate", attrs->dwMinBitRate);
	if (ret != USBG_SUCCESS)
		return ret;

	ret = usbg_write_dec64(frame_path, frame_name, "dwMaxBitRate", attrs->dwMaxBitRate);
	if (ret != USBG_SUCCESS)
		return ret;
